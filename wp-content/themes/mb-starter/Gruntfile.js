module.exports = function(grunt) {
	...
	grunt.initConfig({
		...
		uglify: { /// UGLIFY - minimizes and combines js files. all {src} files into {dest}.
			dist: {
				//src: ['library/js/libs/modernizr.custom.min.js', 'library/slick/slick.js', 'library/js/flowtype.js', 'library/js/jquery-sticky.js', 'library/js/scripts.js'],
				src: ['library/js/libs/modernizr.custom.min.js', 'library/js/libs/matchMedia.js', 'library/js/picturefill.js', 'library/slick/slick.js', 'library/js/flowtype.js', 'library/js/jquery.sticky.js','library/js/scripts.js' /*,'library/js/functions.js'*/],
		...
		sass: { /// SASS - for PreProcessing
			dist: {
				...
				files: {
					...
					'library/css/dashboard.css': 'library/scss/dashboard.scss',       // 'destination': 'source'
					'library/css/login.css': 'library/scss/login.scss',       // 'destination': 'source'
				}
			},
			build: {
				options: {
				...
				files: {
					...
					'../mb-build/library/css/login.css': 'library/scss/login.scss',       // 'destination': 'source'
				}
			}
		},
		watch: { /// WATCH - watches files and performs tasks when there are changes
				src: {
					files: ['library/js/*/*.js', 'library/scss/*.scss', '**/*.php', 'library/slick/*.scss', 'library/slick/*.js'],
		...
		autoprefixer: { /// AUTOPREFIXER
				dist: {
					files: {
						...
						'library/css/dashboard.css': 'library/css/dashboard.css',
						},
						options: {
				...
				build: {
					files: {
						...
						'../mb-build/library/css/login.css': 'library/css/login.css',
						'../mb-build/library/css/dashboard.css': 'library/css/dashboard.css',
						},
						options: {
							browsers: ['last 3 versions']
						},
		...
		copy: { /// THIS IS THE PART OF THE BUILD PROCESS THAT COPIES THE NEEDED FILES TO THE NEW BUILD FOLDER THEME. YOU JUST UPLOAD THAT THEME SINCE IT HAS ONLY THE FILES THAT ARE NEEDED
		  main: {
		    files: [
		      {expand: true, src: ['*.php', '*.ico', '*.png'], dest: '../mb-build/', filter: 'isFile'},
 		      ...
 		      {expand: true, flatten: true, src: ['library/css/svg/**'], dest: '../mb-build/library/css/svg/', filter: 'isFile'},
	...
	// Default task(s).
	grunt.registerTask('default', ['uglify:dist', 'svgmin', 'imagemin', 'imageoptim', 'sass:dist', 'autoprefixer:dist', 'cmq:dist', /*'uncss', 'cssmin' */]);
	grunt.registerTask('images', ['svgmin', 'imagemin','imageoptim']);
	grunt.registerTask('text', ['uglify:dist','sass:dist','autoprefixer:dist', 'cmq:dist']);
	grunt.registerTask('build', ['modernizr','uglify:build', 'sass:build', 'autoprefixer:build', 'cmq:build', 'cssmin:minify', 'cssmin:add_banner', 'svgmin', 'svgsprite', 'imagemin', 'imageoptim','copy']);
